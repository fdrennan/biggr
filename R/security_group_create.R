...
security_group_create <- function(security_group_name = NA, description = 'Add a description') {
  ...
  if(!any(security_group_df$group_name == security_group_name)) {
    create_security <-
    ...
    security_group_id = security_group_df %>%
      filter(group_name == security_group_name) %>%
      pull(group_id)
    }
